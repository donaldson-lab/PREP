...
class MainFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        self.quality_trim_button = wx.Button(processing_panel, -1, 'Quality Trim')
        ...
        self.quality_trim_button.Bind(wx.EVT_LEFT_DOWN, self.quality_trim)
        ...
        quality_box.AddMany([self.quality_trim_button, (self.quality_filter_button, 0, wx.LEFT, 10), (threshold_box, 0, wx.LEFT, 10)])
    ...
    def enter_quality_trim(self, event):
        self.sb.SetStatusText('Trim new Illumina 1.3+ style FASTQ files using PHRED scores with ASCII offset 64, allowing PHRED scores from 0-62. Trim nucleotides with PHRED scores below trimming threshold')
        event.Skip()
    ...
    def quality_trim(self, event):
        in_file = self.fasta_path.GetValue()
        out_file = self.output_path.GetValue()
        threshold = int(self.threshold.GetValue())
        length = 20 
        if out_file == "":
            out_file = 'quality_trimmed.fasta'
        if in_file != "":
            quality_filter.main(in_file, out_file, threshold, length)
        else:
    ...
    def quality_filter(self,event):
        ...
        threshold = 20
        if out_file == "":
            out_file = 'good_quality.fastq'
        if in_file != "":
            fastq_quality.main(in_file, out_file, threshold)
        else:
            dlg = wx.MessageDialog(self, "", "Please enter an input file")
            dlg.ShowModal()
            dlg.Destroy()
